<?php
//
// File generated by ... on the 2016-01-09T23:32:30+0100
...
abstract class SecurityCheckListItem extends SecurityCI
{
  ...
  2 => 'name',
                    ...
                    function DisplayBareRelations(WebPage $oPage, $bEditMode = false)
	                {
		                ...
		                $sBlockId = $sClass.'_Uncomplited';
		                $sBlockHeader = Dict::S('Class:'.$sClass.'/BlockHeader:Uncomplited');
		                $oPage->add('<fieldset>');
		                ...
		                $sBlockId = $sClass.'_Complited';
		                $sBlockHeader = Dict::S('Class:'.$sClass.'/BlockHeader:Complited');
		                $oPage->add('<fieldset>');
		                $oPage->add('<legend>'.$sBlockHeader.'</legend>');
...
class SecurityCheckList extends SecurityCI
{
  ...
  0 => 'name',
                    ...
                    function DisplayBareRelations(WebPage $oPage, $bEditMode = false)
	                {
		                ...
		                $oItemHaveUncomplitedInstanceSet = DBObjectSet::FromScratch($sClass);
		                $oItemHaveOnlyComplitedInstanceSet = DBObjectSet::FromScratch($sClass);
		                $MyID = $this->GetKey();
		                $sExtKeyToMe = 'securitychecklist_id';
		                ...
		                while ($oItem = $oItemSet->Fetch())
		                {
		                    ...
		                    {
		                        $oItemHaveUncomplitedInstanceSet->AddObject($oItem);
		                    }
		                    elseif ($oSetDone->Count() != 0)
		                    {
		                        $oItemHaveOnlyComplitedInstanceSet->AddObject($oItem);
		                    }
		                    else
		                    {
		                ...
		                if ($oItemNotHaveInstanceSet->Count() + $oItemHaveUncomplitedInstanceSet->Count() + $oItemHaveOnlyComplitedInstanceSet->Count() != 0)
		                {
		                    $sCount = ' ('.$oItemNotHaveInstanceSet->Count().' '.$oItemHaveUncomplitedInstanceSet->Count().' '.$oItemHaveOnlyComplitedInstanceSet->Count().')';
		                }
		                else
		                {
		                    $sCount = '';
		                ...
		                $oPage->add('<fieldset>');
		                $oPage->add('<legend>'.$sBlockHeader.'</legend>');
		                $oBlock = DisplayBlock::FromObjectSet($oItemHaveUncomplitedInstanceSet, 'list');
		                $oBlock->Display($oPage, $sBlockId, array('menu' => false));
		                $oPage->add('</fieldset>');
		                ...
		                $oPage->add('<fieldset>');
		                $oPage->add('<legend>'.$sBlockHeader.'</legend>');
		                $oBlock = DisplayBlock::FromObjectSet($oItemHaveOnlyComplitedInstanceSet, 'list');
		                $oBlock->Display($oPage, $sBlockId, array('menu' => false));
		                $oPage->add('</fieldset>');                
